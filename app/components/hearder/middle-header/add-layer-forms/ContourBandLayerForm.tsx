...
import {
  ContourBandLayerFormSchema,
  ContourBandLayerFormSchemaType,
} from "./schema/ContourBandLayerSchema";

function ContourBandLayer({ closeAllDialog }: { closeAllDialog: () => void }) {
  const setDeckLayers = useContext(SetDeckLayersContext);
  const { mainMap } = useMap();
  const form = useForm<ContourBandLayerFormSchemaType>({
    resolver: zodResolver(ContourBandLayerFormSchema),
    defaultValues: {
      id: "contour band layer",
  ...
  const onSubmit = useCallback(
    (values: ContourBandLayerFormSchemaType) => {
      ...
      setDeckLayers((pre) => [
        ...
        new ContourLayer({
          ...
          contours: [
            {
              threshold: [values.startThreshold, values.endThreshold],
      ...
      toast.success("Add contour band layer to map");
      closeAllDialog();
    },
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2">
        ...
        <div className="grid grid-cols-3 grid-rows-1 gap-2">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Start threshold:</FormLabel>
                <FormControl>
                  <Input
                    type="number"
                ...
                <FormDescription>This is your start threshold.</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>End threshold:</FormLabel>
                <FormControl>
                  <Input
                    type="number"
                    placeholder="input end threshold..."
                    {...field}
                  />
                </FormControl>
                <FormDescription>This is your end threshold.</FormDescription>
                <FormMessage />
              </FormItem>
            )}
          />
...
export default ContourBandLayer;
