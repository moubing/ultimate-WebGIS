...
function BoundingPanel() {
  ...
  const generateBounding = useCallback(() => {
    ...
    toast.loading(`generate bounding layer for ${selectedLayerName}`, {
      id: "bounding"
    });
    setTimeout(() => {
      const bounding = caculateBounding(data as FeatureCollection);
      ...
      if (mapSources.find((source) => source.id === newSource.id)) {
        toast.error(`${newSource.id} is already exist`, { id: "bounding" });
        return;
      }
  ...
  useEffect(() => {
    if (isSourceLoaded) {
      ...
      toast.success("bounding success", { id: "bounding" });
    }
