...
function IntersectPanel() {
  ...
  const generateIntersect = useCallback(() => {
    if (!data || !overlayerData) {
      toast.error("data is not loaded yet");
      return;
    }

    toast.loading(`generate intersect layer for ${selectedBaseLayerName}`, {
      id: "intersect"
    });

    setTimeout(() => {
      const intersect = caculateInterscet(
  ...
  return (
    <div className="flex flex-col gap-2 pr-6">
      ...
      <Button onClick={generateIntersect}>Apply</Button>
    </div>
  );
}

export default IntersectPanel;
