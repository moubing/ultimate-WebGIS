...
import CustomSwitch from "@/components/custom-ui/CustomSwitch";
...
type simplifyType = "polygon" | "line";
...
function SimplifyPanel() {
  ...
  const [type, setType] = useState<simplifyType>("polygon");
  ...
  const lineOrPolygonLayers = useMemo(() => {
    if (type === "polygon") {
  ...
  const newFill = useMemo(() => {
    if (type === "polygon") {
      return generateFillLayer(
        `${outputLayerName}-source`,
        `${outputLayerName}-fill`
      );
    }
    return undefined;
  }, [outputLayerName, type]);
  ...
  const generateSimplify = useCallback(() => {
    if (!data) {
      toast.error("data is not loaded yet");
      return;
    }

    toast.loading(`generate simplify layer for ${selectedLayerName}`, {
      id: "simplify"
    });

    setTimeout(() => {
      const simplify = caculateSimplify(
      ...
      if (mapSources.find((source) => source.id === newSource.id)) {
        toast.error(`${newSource.id} is already exist`, { id: "simplify" });
        return;
      }
  ...
  useEffect(() => {
    if (isSourceLoaded) {
      ...
      toast.success("simplify success", { id: "simplify" });
    }
  }, [isSourceLoaded, mapLayers, newFill, newLine, setMapLayers, type]);

  const changeType = useCallback((value: string) => {
    setType(value as simplifyType);
    setSelectedLayerName("none");
  }, []);

  const changeTolerance = useCallback((value: number) => {
    setTolerance(value);
  }, []);

  const changeHighQuality = useCallback((value: boolean) => {
  ...
  return (
    <div className="flex flex-col gap-2 pr-6">
      ...
      <Block>
        ...
        <div className="flex items-center justify-between">
          ...
          <CustomSelect
            ...
            arr={["polygon", "line"]}
            updateSelection={changeType}
          />
        </div>
        <div className="flex items-center justify-between">
          <Field>Tolerance</Field>
          <CustomSlider
            initialOpacity={tolerance}
            min={0.01}
            max={2}
            step={0.01}
            updateOpacity={changeTolerance}
          />
        </div>
        <div className="flex items-center justify-between">
          <Field>High quality</Field>
          <CustomSwitch
            initialState={highQuality}
            updateState={changeHighQuality}
          />
        </div>
      </Block>
      ...
      <Button onClick={generateSimplify}>Apply</Button>
    </div>
  );
}

export default SimplifyPanel;
