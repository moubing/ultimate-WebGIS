...
function DbscanPanel() {
  ...
  const generateBounding = useCallback(() => {
    ...
    toast.loading(`generate dbscan layer for ${selectedLayerName}`, {
      id: "dbscan"
    });

    setTimeout(() => {
      const dbscan = caculateDbscan(
      ...
      if (mapSources.find((source) => source.id === newSource.id)) {
        toast.error(`${newSource.id} is already exist`, { id: "dbscan" });
        return;
      }
  ...
  useEffect(() => {
    if (isSourceLoaded) {
      ...
      toast.success("dbscan success", { id: "dbscan" });
    }
  }, [isSourceLoaded, mapLayers, newCircle, setMapLayers]);

  const changeMaxDistance = useCallback((value: number) => {
    setMaxDistance(value);
  }, []);
  const changeUnit = useCallback((value: string) => {
    setUnit(value as turf.Units);
  }, []);
  const changeMinPoints = useCallback((value: number) => {
    setMinPoints(value);
  }, []);
  ...
  return (
    <div className="flex flex-col gap-2 pr-6">
      <Block>
        ...
        />
        <Field>
          Group dense Point features into clusters using the DBSCAN algorithm.
        </Field>
      </Block>
      <Block>
        ...
        <div className="flex items-center justify-between">
          <Field>Min points</Field>

          <CustomNumberInput
            initialValue={minPoints}
            updateValue={changeMinPoints}
          />
        </div>
      </Block>
      <Block>
